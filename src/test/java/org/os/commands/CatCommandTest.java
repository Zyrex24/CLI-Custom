...
class CatCommandTest {
    ...
    public void test_concat_2_files() throws IOException{
        ...
        File [] files = {new File("test_for_cat1.txt"), new File("test_for_cat2.txt"), new File("test_for_cat3.txt")};
        String content1 = Files.readString(files[0].toPath()).trim();
        String content2 = Files.readString(files[1].toPath()).trim();
        String content3 = Files.readString(files[2].toPath()).trim();
        assertEquals(content1 +content2, content3);
